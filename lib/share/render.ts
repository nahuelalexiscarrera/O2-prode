...
export function renderTemplate(data: ShareData, format: ShareFormat): ReactElement {
  switch (data.template) {
    case "summary":
      return T01_Summary({ data, format }) as ReactElement;
    case "position":
      return T02_Position({ data, format }) as ReactElement;
    case "match":
      return T03_Match({ data, format }) as ReactElement;
    case "achievement":
